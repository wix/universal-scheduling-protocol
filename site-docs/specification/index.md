...
| **Currency amounts** | Minor units / cents, as a JSON number or decimal string | `7500` or `"7500"` = $75.00 |
| **Timezones** | [IANA Time Zone Database](https://www.iana.org/time-zones) | `America/New_York` |

### Integer Encoding

Monetary amount fields — those documented as minor currency units — **MAY** be transmitted as either a JSON number or a decimal string. Consumers **MUST** accept both forms and **MUST** treat them as equal. A producer **SHOULD NOT** vary the form it emits for a given field between responses.

The string form **MUST** be a canonical decimal integer matching `^-?[0-9]+$`: no leading `+`, no exponent, no thousands separator, and no decimal point. Where a field also admits a fractional value — `pricing.deposit.value`, which carries a percentage when `deposit.type` is `percentage` — the string form **MUST** match `^-?[0-9]+(\.[0-9]+)?$`.
