...
**Goal:** Deliver a **UCP-Native Mode** demo in **one 2-week sprint** where a Link agent discovers a Wix Bookings merchant **already listed in a USP registry**, consumes that merchant's `**profile_url`** per [USP Â§6](../specification.md#6-discovery-registry-optional), fetches the **UCP business profile** per [USP Â§7.2](../specification.md#72-profile-registration-in-well-knownucp) and [UCP Profile](https://ucp.dev/latest/specification/overview/), optionally **connects the buyer's calendar** and **filters availability slots** against personal busy times per [USP Â§11.2](../specification.md#112-buyer-calendar-freebusy-extension) (platform-side only; no business changes), runs the [USP Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee) paid flow (UCP `create_checkout` + `complete_checkout` with `dev.usp.services.paid_bookings` + Stripe SPT), receives `**booking.confirmed`** webhook with correlated `**order_id`** ([Â§7.5 step 8](../specification.md#75-checkout-flow-and-atomicity-guarantee), [Â§5.4.1](../specification.md#541-booking-webhooks)), and ends with `**status: completed`**, `**order_id`**, and `**booking.booking_status: confirmed**` â with **no Standalone Mode**, **no `checkout_systems` redirect**, and **no migration** from prior deployments.

**Normative references:** [USP `specification.md](../specification.md)` Â§6 (registry), Â§7 (UCP-Native), `[schemas/paid_bookings.json](../schemas/paid_bookings.json)`, `[schemas/registry.json](../schemas/registry.json)`; [UCP checkout](https://ucp.dev/latest/specification/checkout/), [UCP payment architecture](https://ucp.dev/latest/specification/overview/#payment-architecture), [Stripe UCP/SPT](https://docs.stripe.com/agentic-commerce/protocol).
...
- **Link platform and USP registry are independent ecosystem components** (see [Â§2.1](#21-usp-ecosystem-link-platform-vs-registry)).
...
- [2. Architecture](#2-architecture)
  ...
  - [2.3 Normative protocol alignment map](#23-normative-protocol-alignment-map)
    - [USP registry (Â§6)](#usp-registry-6)
    - [USP UCP-Native business profile (Â§7.2)](#usp-ucp-native-business-profile-72)
    - [USP + UCP paid booking flow (Â§7.5)](#usp--ucp-paid-booking-flow-75)
    - [UCP checkout binding (ucp.dev)](#ucp-checkout-binding-ucpdev)
    - [Inherited from UCP only (USP Â§7.3 â do not reimplement in Standalone layers)](#inherited-from-ucp-only-usp-73--do-not-reimplement-in-standalone-layers)
...
- [5. Track A â Link Agent USP (`linkusp-cli`)](#5-track-a--link-agent-usp-linkusp-cli)
...
- [6. Track B â USP Registry](#6-track-b--usp-registry)
  - [#66 through #70](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/66)
- [7. Track C â Link Platform (Registry Consumer)](#7-track-c--link-platform-registry-consumer)
...
- [8. Track D â Wix Business USP (`usp-impl`)](#8-track-d--wix-business-usp-usp-impl)
...
- [9. Track E â Core UCP + USP Extension (`acp-checkout`)](#9-track-e--core-ucp--usp-extension-acp-checkout)
...
- [10. Track F â Payment with Stripe SPT](#10-track-f--payment-with-stripe-spt)
...
- [Out of scope â future version](#out-of-scope--future-version)
...
Field names in the following detailed steps description refer to  `[paid_bookings.json](../schemas/paid_bookings.json)` `BookingContext`, UCP checkout request fields from [USP Â§7.4](../specification.md#74-paid-bookings-extension-schema), and upstream schemas `[registry.json](../schemas/registry.json)`, `[catalog.json](../schemas/catalog.json)`, `[availability.json](../schemas/availability.json)`. **Registry snapshot fields are discovery hints only** unless marked authoritative; live catalog from step 6 is authoritative for checkout per [Â§6.3](../specification.md#63-service-search---post-registrysearch_services).

1. **Registry service search** (`Agent` â `Registry`): The Link agent calls
  `POST /registry/search_services` with at least one filter (e.g. `query` plus optional `location`/`verticals`/`categories`) per [USP Â§6.3](../specification.md#63-service-search---post-registrysearch_services).  
...
2. **Search results** (`Registry` â `Agent`): The registry returns one or more `[ServiceSearchResult](../schemas/registry.json)` hits.
...
3. **Fetch UCP business profile** (`Agent` â `UCP`): The agent issues `GET {profile_url}` using the **full profile document URL** from the registry hit (e.g. `https://{host}/.well-known/ucp`), not site origin plus an appended path.
  **Why:** Per [USP Â§6.1](../specification.md#61-business-registration---post-registrybusinesses) and [Â§7.2](../specification.md#72-profile-registration-in-well-knownucp), merchant capabilities, REST endpoints, and payment handlers are advertised in the UCP profile; the agent must load that document before any booking-time calls.
...
4. **Profile document** (`UCP` â `Agent`): `acp-checkout` serves the UCP profile with required capabilities (`dev.ucp.shopping.checkout`, `dev.usp.services.`*), `ucp.services` endpoint map, and `payment_handlers` (including Stripe SPT).
  **Why:** The agent validates that `paid_bookings` **extends** `checkout` per [Â§2.4](#24-what-paid_bookings-extends-checkout-means), resolves USP and UCP base URLs, and selects the correct payment handler before mutating checkout.
   **Fields obtained â later use:**
...
5. **Live catalog fetch** (`Agent` â `USP`): The agent calls `GET /services/{service_id}` on the merchant USP catalog endpoint resolved from the profile.
  **Why:** [USP Â§6.3](../specification.md#63-service-search---post-registrysearch_services) requires live catalog at booking time; the registry snapshot is non-authoritative for price, `service_type`, and policies used in checkout.
...
6. **Service record** (`USP` â `Agent`): `usp-impl` returns the current `[Service](../schemas/catalog.json)`.
  **Why:** Availability queries and `create_checkout` must reflect server-side catalog state; the merchant re-validates price at create and returns `price_mismatch` if the agent sends a stale amount ([Â§7.4](../specification.md#74-paid-bookings-extension-schema)).
   **Fields obtained â later use:**

  | Field obtained                                         | Used in step(s)                                                      | Maps to on `create_checkout` (step 12)                                                                                      |
  | ------------------------------------------------------ | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
  | `id`                                                   | 9, 12                                                                | `booking.service_id`; `line_items[].item.id` (must match per [Â§7.4](../specification.md#74-paid-bookings-extension-schema)) |
...
7. **Buyer calendar gate** (`Agent` internal, platform-only): Before querying business availability, the agent asks the buyer whether to check their personal calendar for conflicts, then completes the calendar gate.
  **Why:** [USP Â§11.2](../specification.md#112-buyer-calendar-freebusy-extension) enables platform-side free/busy cross-referencing so buyers only see mutually free times; the business is not involved and never receives calendar data. Matches `linkusp flow calendar ask|connect|skip` hard gate and the ds-general USP subagent Scenario 2 Step 1 (`customer_calendar_connected` / `customer_calendar_skipped`).
   ...
   **Demo path:** `linkusp flow calendar ask` â user chooses â `calendar connect` (local Google OAuth; agent relays `auth_url`, does not open browser) or `calendar skip`. **Production path:** `link-cli calendar connect` via Link-hosted calendar service ([linkusp-cli DESIGN Â§3](https://github.com/yahalomran/linkusp-cli/blob/main/DESIGN.md#3-linkusp-as-google-calendar-client)).
8. **Fetch buyer busy blocks** (`Agent` â calendar provider, optional): When `calendar_state: connected`, the platform fetches opaque `[BusyBlock](../schemas/calendar_freebusy.json)` intervals for the availability time window.
  **Why:** Â§11.2.6 filtering requires `{start, end}` pairs only; no event titles, attendees, or locations. Overlap rule: `slot.start < block.end AND block.start < slot.end` (touching boundaries do not overlap).
...
9. **Availability query** (`Agent` â `USP`): The agent calls `POST /availability/query` for the chosen `service_id` and time window.
  **Why:** [USP Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee) step 2 requires confirming bookable slots before checkout; this selects a concrete slot for the `paid_bookings` extension. **No changes to the business request** when calendar is connected ([Â§11.2.6](../specification.md#1126-integration-with-availability-query)).
...
10. **Available slots** (`USP` â `Agent`): `usp-impl` returns matching `[TimeSlot](../schemas/availability.json)` entries.
  **Why:** Checkout requires a schema-valid `SlotReference` tied to real capacity; without this response the agent cannot build a valid `booking` object on `create_checkout`.
   **Fields obtained â later use** (from business response, before platform filter):
...
11. **Platform slot filter and selection** (`Agent` internal): When `calendar_state: connected`, the agent removes slots overlapping step 8 `busy_blocks` per Â§11.2.6, then the buyer picks one remaining slot (human gate; never auto-pick).
  **Why:** Presents only mutually free times; surfaces filtered-out ranges in agent UX so the buyer can adjust their calendar if needed (linkusp `calendar_filtered` + `slots_filtered` in flow JSON).
   **Fields obtained â later use:** selected slot fields from step 10 (post-filter) feed step 12 `booking.slot`.
12. **Create checkout** (`Agent` â `UCP`): The agent calls UCP `POST create_checkout` with the `dev.usp.services.paid_bookings` extension.
  **Why:** UCP-Native paid booking starts as a standard UCP checkout session extended for scheduling; this is [USP Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee) step 4.
  ...
  | `line_items[].item.price`                            | 6 (`Service.pricing.amount`) or 10 (`TimeSlot.pricing` if variable) | Must match live catalog ([Â§7.4](../specification.md#74-paid-bookings-extension-schema))                         |
  | `line_items[].quantity`                              | Agent                                                               | Demo: `1`                                                                                                       |
  | `currency`                                           | 6 (`Service.pricing.currency`) or 4 (profile fallback)              |                                                                                                                 |
  | `buyer.email`, `buyer.first_name`, `buyer.last_name` | Link account (platform; not a numbered diagram step)                | Required for `ready_for_complete` ([Â§7.5 step 4](../specification.md#75-checkout-flow-and-atomicity-guarantee)) |
  ...
  | `booking.hold_id`                                    | â                                                                   | **Not used** (demo `holds: false`)                                                                              |
  | `Idempotency-Key` (header)                           | Agent-generated                                                     | [Â§7.3](../specification.md#73-inherited-infrastructure) / UCP idempotency                                       |
...
13. **Pending booking** (`UCP` â `USP`): `acp-checkout` invokes internal `CreatePendingBooking` gRPC on `usp-impl`, reserving the slot in `pending` state.
...
14. **Checkout ready** (`UCP` â `Agent`): UCP returns checkout `status: ready_for_complete` plus booking and payment metadata.
  **Why:** The agent needs the checkout session handle, correlated booking id, and payment handler binding before acquiring SPT and calling `complete_checkout`.
  ...
  | `booking.actions[]` (if present)  | â                    | **Demo avoids:** would require [Â§7.5 step 5](../specification.md#75-checkout-flow-and-atomicity-guarantee) before payment |

15. **Acquire SPT** (`Agent` â `Stripe`): The Link platform obtains a Shared Payment Token via Stripe's UCP/SPT flow.
...
16. **SPT credential** (`Stripe` â `Agent`): Stripe returns the SPT credential bound to the checkout context.
...
17. **Complete checkout** (`Agent` â `UCP`): The agent calls UCP `POST complete_checkout` with the SPT and checkout session reference.
  **Why:** Atomic payment-plus-confirmation gate per [USP Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee) steps 6-7.
    **Fields consumed (this request):**
  ...
  | `Idempotency-Key` (header)         | Agent-generated    | UCP idempotency per [Â§7.3](../specification.md#73-inherited-infrastructure)              |
...
18. **Charge payment** (`UCP` â `Stripe`): `acp-checkout` charges via `StripeSptProviderAdapter`.
...
19. **Finalize booking** (`UCP` â `USP`): On successful charge, `acp-checkout` calls `FinalizeBookingOnPayment` gRPC.
  **Why:** Scheduling confirmation must follow payment success; uses `confirmation_mode` from step 6 policies (`auto` in demo).
    ...
    **Fields obtained (agent-visible):** deferred to step 20.
20. **Checkout completed** (`UCP` â `Agent`): UCP returns terminal checkout state.
  **Why:** Synchronous success signal for demo assertions per [USP Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee) atomicity guarantees.
    **Fields obtained â later use:**
...
21. **Booking webhook** (`USP` â `Agent`): `usp-impl` asynchronously `POST`s `booking.confirmed`.
  **Why:** [USP Â§7.5 step 8](../specification.md#75-checkout-flow-and-atomicity-guarantee) and [Â§5.4.1](../specification.md#541-booking-webhooks) durable notification; signature verified per [Â§10.1.1](../specification.md#1011-webhook-security).
    **Fields obtained â later use:**
  ...
  | Webhook signature headers | Agent verification | [Â§10.1.1](../specification.md#1011-webhook-security) authenticity |
...
1. **Webhook first:** Process `booking.confirmed` idempotently (dedupe on `event_id` per Â§9.2.3). The payload already carries `booking_id` and `order_id`, so you do not need step 20 to arrive first. The agent already had `booking_id` from step 14 (`create_checkout`).
...
- Â§9.2.3 only guarantees **causal order among events for the same booking** (e.g. `booking.confirmed` before `booking.canceled`), not ordering relative to the checkout HTTP response.
...
- [ ] Link discovers the service and merchant via `**POST /registry/search_services` only** (no hardcoded merchant URL in agent config; no `search_business` in demo path); then `**GET /services/{service_id}`** for live catalog per [Â§6.3](../specification.md#63-service-search---post-registrysearch_services) before availability and checkout.
- [ ] **Buyer calendar gate** completes before availability (`calendar connect` or `calendar skip` per [Â§11.2](../specification.md#112-buyer-calendar-freebusy-extension)); when connected, slots are filtered platform-side against opaque busy blocks and conflicting times are not offered.
- [ ] Service search uses at least one filter per [USP Â§6.3](../specification.md#63-service-search---post-registrysearch_services) (e.g. `query` plus optional `verticals`/`categories`); no client-side post-filter by `deployment_mode` or payment handlers in the demo ([#94](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/94) is the correct solution when agents need those filters).
...
- [ ] UCP-Native profile includes required capabilities per [USP Â§7.2](../specification.md#72-profile-registration-in-well-knownucp); `dev.usp.services.paid_bookings` declares `"extends": "dev.ucp.shopping.checkout"` ([Â§2.4](#24-what-paid_bookings-extends-checkout-means)); no `checkout_systems` field ([USP Â§7.1](../specification.md#71-overview-and-when-to-use)).
...
- [ ] Atomic completion per [USP Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee): `booking.booking_status: confirmed` when `confirmation_mode` is `auto` and checkout `status: completed`.
- [ ] `**booking.confirmed` webhook** received with `booking_id` and `order_id` matching the completed checkout ([Â§5.4.1](../specification.md#541-booking-webhooks), `[schemas/webhook_event.json](../schemas/webhook_event.json)`); signature verified per [Â§10.1.1](../specification.md#1011-webhook-security).
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------------------------- |
| **USP registry**              | Federated directory; `POST /registry/businesses`, search APIs; returns `profile_url` per entry         | **Yes** â business registration is a registry-side process       | `universal-scheduling-protocol` |
| **Link platform** (`linkusp`) | One USP-enabled AI platform; cold-start discovery via **any** configured registry; profile consumption | **No** â Link never calls `POST /registry/businesses`            | `linkusp-cli` + Link services   |
...
**Registration (out of Link scope):** Per [USP Â§6.1](../specification.md#61-business-registration---post-registrybusinesses), a registrar (registry operator, business SaaS, or partner â **not** a consumer platform) calls `POST /registry/businesses` with a `[RegistrationRequest](../schemas/registry.json)` body including:

- `profile_url` â **full URL of the profile document** (for UCP-Native: `https://{host}/.well-known/ucp`)
- `deployment_mode: "ucp_native"`
- `name`, `verticals`, `categories`, `timezone`, and `location` when required

The registry **MUST** validate that `GET profile_url` returns a valid UCP profile ([Â§6.1](../specification.md#61-business-registration---post-registrybusinesses)). Authoritative business capabilities live in that profile, not in the registration payload. Link is not involved in registration.

**Discovery (Link / any platform scope):** Per [USP Â§6.3](../specification.md#63-service-search---post-registrysearch_services) and [Â§6.7](../specification.md#67-registry-governance), once registered:
...
3. `GET profile_url` â fetch UCP business profile ([USP Â§7.2](../specification.md#72-profile-registration-in-well-knownucp); discovery inherited from UCP per [Â§7.3](../specification.md#73-inherited-infrastructure)).
4. Match required capabilities and versions; verify `paid_bookings` **extends** `checkout` per [Â§2.4](#24-what-paid_bookings-extends-checkout-means) (both capabilities present; `extends` field equals `dev.ucp.shopping.checkout`).
...
7. `**GET /services/{service_id}`** on the merchant USP catalog endpoint ([Â§3.12.3](../specification.md#3123-get-service---get-servicesservice_id)) â live catalog for booking-time decisions per [Â§6.3](../specification.md#63-service-search---post-registrysearch_services) (registry hit is a non-authoritative snapshot).
8. **Buyer calendar gate** (platform-only, [Â§11.2](../specification.md#112-buyer-calendar-freebusy-extension)): ask buyer to connect personal calendar for conflict checking or skip; hard gate before availability (`linkusp flow calendar ask|connect|skip`; ds-general USP subagent Scenario 2 Step 1).
9. Use the live `Service` object (`type` â `booking.service_type`, `pricing`, `policies`) plus registry `service_id` for availability and checkout (`create_checkout` re-validates catalog price server-side).
10. Perform UCP auth, consent, and identity linking per [Â§7.3](../specification.md#73-inherited-infrastructure) before mutating checkout.
...
Link Agent (consumer only)
    ...
    |
    +-- GET /services/{service_id} ... usp-impl (REST, live catalog per Â§6.3)
    |
    +-- [platform] calendar gate + free/busy filter (Â§11.2; no business call)
    |
...
#### USP registry ([Â§6](../specification.md#6-discovery-registry-optional))


| Requirement                                                              | Spec                        | Plan enforcement                                                               |
| ------------------------------------------------------------------------ | --------------------------- | ------------------------------------------------------------------------------ |
| Registry optional; cold-start only                                       | Â§6 intro                    | Demo uses registry; direct `profile_url` also valid but not used               |
| `profile_url` is full profile document URL                               | Â§6.1, `RegistrationRequest` | [#68](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/68), [#69](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/69), [#72](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/72), [#73](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/73)                                                 |
| `deployment_mode: ucp_native` on register                                | Â§6.1                        | [#69](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/69)                                                                         |
| Demo discovery via service search only                                   | Â§6.3                        | [#72](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/72), [#65](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/65), [#75](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/75)                                                         |
| `deployment_mode` on service search hits (`RegistryBusinessRef`)         | Â§6.3 response               | [#72](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/72) returns metadata; [#94](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/94) registry request filters when needed           |
| Registry validates reachable profile before accept                       | Â§6.1 MUST                   | [#68](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/68)                                                                         |
| Service search requires â¥1 filter                                        | Â§6.3 MUST                   | [#67](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/67), [#72](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/72)                                                                 |
| Fetch live catalog at booking time (registry snapshot non-authoritative) | Â§6.3 MUST                   | [#62](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/62) `GET /services/{service_id}` after profile                              |
| Platform calendar free/busy filter before slot selection                 | Â§11.2                       | [#63](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/63) |
| Registry `usp` envelope describes **registry**, not business             | Â§6.1                        | [#66](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/66) implementers note                                                       |
| Federated registries; business may register with multiple                | Â§6.7                        | Link uses configurable `USP_REGISTRY_URL` ([#71](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/71))                             |
| Platforms search only; never register                                    | Â§6.2â6.3 (consumer ops)     | Track C; no `POST /registry/businesses` in Link                                |


#### USP UCP-Native business profile ([Â§7.2](../specification.md#72-profile-registration-in-well-knownucp))


| Requirement                                                                              | Spec                                                              | Plan enforcement        |
| ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------- |
| Single `/.well-known/ucp`; no `/.well-known/usp`                                         | Â§7.1                                                              | [#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80); greenfield demo |
| No `checkout_systems` in profile                                                         | Â§7.2, Â§7.1                                                        | [#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80), [#73](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/73)          |
| `dev.usp.services` service entry with REST endpoint                                      | Â§7.2                                                              | [#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80)                  |
| Capabilities: `catalog`, `availability`, `bookings`, `paid_bookings`                     | Â§7.2                                                              | [#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80)                  |
| `paid_bookings` extends `dev.ucp.shopping.checkout` (profile `extends` field + protocol) | Â§7.2, Â§7.4, [Â§2.4](#24-what-paid_bookings-extends-checkout-means) | [#73](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/73), [#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80), [#81](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/81)  |
| `dev.ucp.shopping.checkout` capability present                                           | Â§7.2                                                              | [#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80), [#73](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/73)          |
| `availability` may declare `holds: false`                                                | Â§4, demo scope                                                    | [#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80)                  |


#### USP + UCP paid booking flow ([Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee))


| Step | Normative action                                           | Plan task                                                                                                                                                                                                                                                                        |
| ---- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | `POST /services/list`                                      | Out of scope â demo uses registry `search_services` for cold-start, then `GET /services/{service_id}` for live catalog ([Â§6.3](../specification.md#63-service-search---post-registrysearch_services); [merchant-direct list](#merchant-direct-catalog-discovery) remains future) |
| 1b   | `GET /services/{service_id}`                               | [#62](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/62) â booking-time catalog hydration per [Â§3.12.3](../specification.md#3123-get-service---get-servicesservice_id)                                                                                                                                                             |
| 1c   | Buyer calendar gate + platform free/busy filter            | [#63](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/63) â [Â§11.2](../specification.md#112-buyer-calendar-freebusy-extension); no change to `POST /availability/query`                                                                                     |
...
| 8    | Webhook `booking.confirmed` with `order_id`                | [Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee) step 8                                                                                                                                                                                                      |
...
| `Idempotency-Key` on create (and complete)                                                       | UCP idempotency, USP Â§7.3               | [#86](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/86), existing create guard |
...
| Line item `item.price` in minor units; MUST match catalog                                        | USP Â§7.4                                | [#82](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/82)                        |
| `continue_url` + `get_checkout` poll on 3DS / `complete_in_progress`                             | UCP + USP Â§7.5                          | [#102](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/102) (out of demo scope) |
...
| `order.id` (UCP) mapped to USP `order_id` / webhook correlation                                  | UCP complete checkout + USP Â§5.4.1      | [#98](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/98)                      |
...
| Cancel atomicity: checkout `canceled`, booking `canceled`                                        | USP Â§7.5                                | [#85](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/85)                        |


#### Inherited from UCP only (USP Â§7.3 â do not reimplement in Standalone layers)

Discovery after `profile_url` fetch, capability negotiation, versioning, RFC 9457 errors, idempotency, webhooks, identity, consent, OAuth, TLS â **use UCP bindings**, not `/.well-known/usp` or Standalone `USP-Agent` negotiation.
...
In `GET {profile_url}` â `ucp.capabilities`, a paid UCP-Native demo merchant **MUST** declare **both**:
...
The `paid_bookings` entry **MUST** include `"extends": "dev.ucp.shopping.checkout"` per [USP Â§7.2](../specification.md#72-profile-registration-in-well-knownucp):
...
4. Demo also requires `catalog`, `availability`, and `bookings` USP capabilities per Â§7.2.

**Not sufficient:** `bookings` alone, or `paid_bookings` without the base `checkout` capability, or `paid_bookings` whose `extends` points at a different capability. Free-only merchants correctly omit **both** `checkout` and `paid_bookings` ([Â§7.2](../specification.md#72-profile-registration-in-well-knownucp)); they are out of demo scope.

#### Protocol meaning (what the agent does after verification)

Per [USP Â§2.4](../specification.md#24-core-constructs), an **extension** layers extra fields onto a base capability schema. `[paid_bookings.json](../schemas/paid_bookings.json)` uses UCP schema composition (`allOf` + `$defs` keyed by `dev.ucp.shopping.checkout`) to add a `booking` object to the checkout payload ([Â§7.4](../specification.md#74-paid-bookings-extension-schema)).
...
- Acquire SPT from `payment_handlers` and pass it on `complete_checkout` for atomic payment + booking per [Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee).
...
**Critical path:** [#76](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/76) â [#77](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/77)/032 â [#84](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/84) â [#88](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/88)/053 â [#87](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/87) â [#91](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/91) (webhook emit) â [#92](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/92) (webhook receive) â [#69](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/69) (registry listing + service index) â [#72](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/72)/022 (Link service search discovery) â [#65](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/65) demo.
...
In-scope gaps only. Excluded work (holds, Standalone, mixed cart, MCP, registry capability filters, etc.) is listed in [Out of scope â future version](#out-of-scope-future-version) without matrix IDs.
...
**Out of scope (no matrix ID):** UCP auth/identity, trusted UI, 3DS escalation, full UCP commerce surface, and other non-demo conformance items are tracked in [#102](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/102) under [Out of scope â future version](#out-of-scope--future-version).

---

## 5. Track A â Link Agent USP (`linkusp-cli`)
...
1. Add `UcpNativeProfile` / `UcpNativeContext` models for the UCP profile document returned by `GET {profile_url}` ([USP Â§7.2](../specification.md#72-profile-registration-in-well-knownucp)).
...
**Why:** [USP Â§7.7.2](../specification.md#772-paid-service-flow-ucp-checkout) creates a pending booking via UCP `create_checkout` with the `paid_bookings` extension, not `POST /bookings`.
...
2. Implement `create_checkout(line_items, buyer, booking)` with `Idempotency-Key` header per [UCP idempotency](https://ucp.dev/latest/specification/overview/) and [USP Â§7.3](../specification.md#73-inherited-infrastructure).
...
**Why:** After registry `search_services` yields `service_id` and profile fetch resolves the USP endpoint, the agent **MUST** load live catalog per [Â§6.3](../specification.md#63-service-search---post-registrysearch_services) via `GET /services/{service_id}` before availability and checkout (replaces normative Â§7.5.1 list for the known id).

**What:**

1. Implement `get_service(service_id)` â `GET /services/{service_id}` per [Â§3.12.3](../specification.md#3123-get-service---get-servicesservice_id); parse full `[Service](../schemas/catalog.json)` (`type`, `pricing`, `policies`, `duration`).
...
4. Build checkout `line_items` and `booking` extension from live `Service` (`name`, `pricing`, `type` â `service_type`, optional `policies.confirmation_mode`); rely on merchant `price_mismatch` if line item diverges from server catalog at create time.
5. **Do not** call `POST /services/list` or `POST /availability/holds` (out of scope).

**Order:** registry hit â profile â `**GET /services/{service_id}`** â calendar gate â `POST /availability/query` (+ platform filter if connected) â checkout.
...
4. **Production mode (stub OK for sprint):** delegate to `link-cli calendar connect` / Link calendar service per linkusp-cli DESIGN Â§3.
...
7. **MUST NOT** send buyer calendar data to the business ([Â§11.2.6](../specification.md#1126-integration-with-availability-query)).
...
2. E2E steps mapped to [USP Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee) / [Â§7.7.2](../specification.md#772-paid-service-flow-ucp-checkout):
  ...
  | Catalog  | `GET /services/{service_id}` â live `[Service](../schemas/catalog.json)` per [Â§6.3](../specification.md#63-service-search---post-registrysearch_services) (replaces Â§7.5.1 list for known id)   |
  | Calendar | `flow calendar skip` (headless CI via `--calendar-skip`) or `flow calendar connect` (interactive demo with Google OAuth) per [Â§11.2](../specification.md#112-buyer-calendar-freebusy-extension) |
  | Â§7.5.2   | `POST /availability/query` for that `service_id`; platform filters slots when calendar connected                                                                                                |
  | Â§7.5.4   | UCP `create_checkout` + `booking` extension                                                                                                                                                     |
  | Â§7.5.6   | Acquire SPT from `payment_handlers`                                                                                                                                                             |
  | Â§7.5.7   | UCP `complete_checkout`                                                                                                                                                                         |
  | Â§7.5.8   | Await `booking.confirmed` webhook ([#92](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/92)); assert `order_id` + `booking_id` match checkout                                                   |

   Skip Â§7.5.3 (holds) and Â§7.5.5 (non-payment actions) for demo. Demo does **not** call `search_business`.
...
**Why:** Demo cold-start: agent discovers the Wix merchant without a hardcoded URL ([USP Â§6](../specification.md#6-discovery-registry-optional)).
...
**Why:** Demo cold-start uses `**search_services` only** ([Â§6.3](../specification.md#63-service-search---post-registrysearch_services)); registry must index catalog services from registered UCP-Native merchants so Link can find a bookable service by query.
...
1. On register/update: `GET {profile_url}` with 10s timeout (URL is already the profile document per [Â§6.1](../specification.md#61-business-registration---post-registrybusinesses)).
...
3. Return `profile_unreachable` or `validation_error` per [USP Â§9.4](../specification.md#94-error-code-mapping) on failure.
4. Registry response `usp` envelope describes **registry** capabilities (`dev.usp.discovery.registry`), not the business ([Â§6.1](../specification.md#61-business-registration---post-registrybusinesses)).
...
## 7. Track C â Link Platform (Registry Consumer)
...
# linkusp/registry_client.py â consumer only (demo uses search_services)
def search_services(query: str, verticals: list[str] | None = None) -> list[ServiceSearchResult]:
    # ServiceSearchRequest: at least one filter required (Â§6.3)
    body = {"query": query}
    if verticals:
...
1. `discover_service_via_registry(query, verticals=None)` â `POST /registry/search_services` with `query` and/or `verticals`/`categories` per [Â§6.3](../specification.md#63-service-search---post-registrysearch_services).
...
**Why:** Per USP Â§7, after registry returns `profile_url`, the platform fetches the profile and determines what the business supports ([discovery](#21-usp-ecosystem-link-platform-vs-registry) steps 4-7). Paid demo path requires explicit `**paid_bookings` extends `checkout`** verification ([Â§2.4](#24-what-paid_bookings-extends-checkout-means)).

**What:**

1. `GET {profile_url}` â `profile_url` from registry **is** the profile document URL ([Â§6.1](../specification.md#61-business-registration---post-registrybusinesses)); do not append `/.well-known/ucp`.
2. Match required capabilities per [Â§7.2](../specification.md#72-profile-registration-in-well-knownucp): `dev.ucp.shopping.checkout`, `dev.usp.services.catalog`, `availability`, `bookings`, `paid_bookings`.
3. **Verify extension relationship** per [Â§2.4](#24-what-paid_bookings-extends-checkout-means):
...
4. Read `payment_handlers` from profile (UCP); UCP-Native has **no** `checkout_systems` ([Â§7.1](../specification.md#71-overview-and-when-to-use)).
...
**Why:** UCP-Native mode inherits UCP auth, consent, and identity linking ([USP Â§7.3](../specification.md#73-inherited-infrastructure)). Link must perform whatever token exchange the demo merchant profile requires before scheduling/checkout calls.
...
1. Test: `search_services` â profile fetch â capability match â endpoint extraction â `**GET /services/{service_id}`** returns live `Service` with `type` and `pricing`; assert `service_id` present.
...
**Why:** Demo must complete [USP Â§7.5 step 8](../specification.md#75-checkout-flow-and-atomicity-guarantee): platform receives `booking.confirmed` and correlates `order_id` with the UCP checkout ([G-12](#4-gap-to-workstream-matrix)).
...
3. Verify inbound webhook signature using `signing_keys` from business UCP profile ([Â§10.1.1](../specification.md#1011-webhook-security)).
4. Parse `[BookingEvent](../schemas/webhook_event.json)`; idempotent handling on `event_id` per [Â§9.2.3](../specification.md#923-webhook-notifications).
...
## 8. Track D â Wix Business USP (`usp-impl`)
...
**Why:** Atomic `complete_checkout` must confirm booking only after successful SPT charge ([USP Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee)).
...
**Why:** UCP `cancel_checkout` must cancel the pending booking ([USP Â§7.5 cancel](../specification.md#75-checkout-flow-and-atomicity-guarantee)).
...
3. Sign payload per [Â§10.1.1](../specification.md#1011-webhook-security) (RFC 9421); publish `signing_keys` on business UCP profile ([#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80) or this task).
...
5. Delivery is **async best-effort** (not part of atomic `complete_checkout`); retry per [Â§9.2.3](../specification.md#923-webhook-notifications).
6. Idempotent on `event_id` per booking confirmation.

---

## 9. Track E â Core UCP + USP Extension (`acp-checkout`)
...
1. Extend `UcpCapabilities.scala`: add `dev.usp.services.catalog`, `availability`, `bookings`, `paid_bookings` (with `extends: dev.ucp.shopping.checkout` per [Â§7.2](../specification.md#72-profile-registration-in-well-knownucp)).
...
**Why:** USP Â§7.5 defines derivation from UCP checkout status to `booking.booking_status`.

**What:**

1. In `UcpMappers.mapEcomCheckoutToUcp`: implement derivation rules (`completed` â `confirmed` if auto; `canceled` â `canceled`; else `pending`).
...
**Track E scope:** Implementers and adapters must map UCP `order.id` to USP/webhook `order_id`; update spec Â§7.5 / Â§5.4.1 normative text and examples. See issue for acceptance criteria.
...
**Track E scope:** Demo merchant profile from [#80](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/80) must include `spec` + `schema` URLs on every capability per UCP profile rules; spec Â§7.2 updated accordingly.

---

## 10. Track F â Payment with Stripe SPT
...
1. Spike with Payments platform: SPT in `PaymentCredential` â Cashier â Stripe PaymentIntent with `shared_payment_granted_token`.
...
**Track F scope:** Demo merchant profile and checkout `payment_handlers` from [#89](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/89) must match UCP-conformant handler shape; spec Â§7.2 / Â§7.4 aligned with [UCP payment architecture](https://ucp.dev/latest/specification/overview/#payment-architecture).
...
2. Order: validate slot â charge SPT â mark completed â finalize booking.
...
**Why:** Some test cards require 3DS; full UCP maps this to `requires_escalation` + buyer handoff. **Not in the 2-week demo** â demo uses non-3DS test cards only.
...
| Fix integration defects                           | Whichever track owns the failure | â                                              |
...
1. **[#65](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/65)** passes using registry discovery only (no hardcoded merchant URL in Link); demo flow: registry service search â profile â `**GET /services/{service_id}`** (Â§6.3 live catalog) â calendar gate â Â§7.5.2 availability (+ platform filter when connected) â steps 4, 6, 7, **8** (webhook `order_id` correlation).
...
5. **[#84](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/84)** atomic complete per [Â§7.5](../specification.md#75-checkout-flow-and-atomicity-guarantee): failed charge never confirms booking.
...
## Out of scope â future version

The following are **explicitly excluded** from the 2-week UCP-Native demo:

### Merchant-direct catalog discovery

- `POST /services/list` per [USP Â§7.5.1 / Â§3.12.1](../specification.md#3121-list-services---post-serviceslist) when the platform discovers services **from the merchant profile** without registry `search_services` (normative merchant-direct path; e.g. `linkusp flow business use` then catalog browse)
- Full catalog browse and catalog free-text search via list (demo uses registry search for cold-start, then a single `**GET /services/{service_id}`** per selected hit â in demo scope)
...
- **Population:** snapshot from `GET {profile_url}` at register/update and on periodic re-index (catalog feed or poll per [Â§6.3](../specification.md#63-service-search---post-registrysearch_services)); store `profile_indexed_at` / `last_validated_at` on `RegistryEntry` and service hits.
- **Staleness:** search results remain non-authoritative; platforms **MUST** still fetch live profile at booking time per [Â§6.3](../specification.md#63-service-search---post-registrysearch_services). Document max index age and when filters are best-effort vs strict.
...
- **Schema/spec:** extend `[schemas/registry.json](../schemas/registry.json)` (`RegistrationRequest`, `RegistryEntry`, `BusinessSearchRequest`, `ServiceSearchRequest`) and USP Â§6; linkusp consumer updates in a follow-on sprint ([#94](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/94)).
...
- **Auth, identity, consent** â OAuth, identity linking, checkout scopes ([#74](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/74) demo stub only)
- **Trusted UI / agent handoff** â UCP platform guideline that checkout finalization uses a trusted, deterministic UI (unless AP2 mandates apply); demo is fully agent-driven via `linkusp demo ucp-native`
- **3DS / payment escalation** â `requires_escalation`, `continue_url`, buyer handoff ([#93](https://github.com/wix-private/universal-scheduling-protocol-spec/issues/93) deferred)
- **Full UCP commerce surface** â `dev.ucp.shopping.order`, fulfillment extension, mixed cart, AP2 mandates, MCP/A2A/embedded transports
...
- Link-hosted calendar OAuth service (`calendar.link.com`) and LPOS token vault ([linkusp-cli #4](https://github.com/yahalomran/linkusp-cli/issues/4)) â demo uses local Google OAuth or `calendar skip`
...
- [USP Â§6 â Discovery Registry](../specification.md#6-discovery-registry-optional) Â· `[schemas/registry.json](../schemas/registry.json)`
- [USP Â§7 â UCP-Native Mode](../specification.md#7-ucp-native-mode) Â· [Â§7.2 Profile](../specification.md#72-profile-registration-in-well-knownucp) Â· [Â§7.5 Checkout flow](../specification.md#75-checkout-flow-and-atomicity-guarantee) Â· [Â§7.7.2 Paid service flow](../specification.md#772-paid-service-flow-ucp-checkout)
- `[schemas/paid_bookings.json](../schemas/paid_bookings.json)`
- [UCP overview](https://ucp.dev/latest/specification/overview/) Â· [UCP checkout](https://ucp.dev/latest/specification/checkout/) Â· [UCP checkout REST](https://ucp.dev/latest/specification/checkout-rest/) Â· [UCP payment architecture](https://ucp.dev/latest/specification/overview/#payment-architecture)
- [linkusp-cli](https://github.com/yahalomran/linkusp-cli)
- [Stripe UCP](https://docs.stripe.com/agentic-commerce/protocol) Â· [SPT](https://docs.stripe.com/agentic-commerce/concepts/shared-payment-tokens)
