...
| `105-cancel-checkout-abandoned-booking` | After `cancel_checkout`, the USP-visible booking is terminal with `reason_code: checkout_abandoned`, a subsequent `confirm` is refused with `invalid_transition` at HTTP 200, and a later read returns the same `canceled_at`. |
