...
| U3 | Align schemas/booking.json Booking.expires_at description | schemas/booking.json | python3 -m json.tool schemas/booking.json >/dev/null; grep expires_at description | done |
| U4 | Mirror the rule in site-docs/specification/booking.md | site-docs/specification/booking.md | grep -n "Present for\\|MAY omit\\|advertis" site-docs/specification/booking.md | done |
| U5 | Append CHANGE_LOG.md entry (real date/git identity, no GitHub citations) | CHANGE_LOG.md | test -f CHANGE_LOG.md | done |
