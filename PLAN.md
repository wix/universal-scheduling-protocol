...
| U3 | Point MCP category filter parameters at the canonical schema | openrpc/usp-mcp.json | Parse OpenRPC JSON and confirm OpenAPI/OpenRPC search filters resolve through schema `$ref`s | done |
| U4 | Add the required change log entry and run repository verification | CHANGE_LOG.md | Run schema/binding validation and the documentation build available in the repository | done |
